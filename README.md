...
- 工具箱加水印、转比例（留白、智能裁剪、本机模板）和智能抠图（三种背景、边缘精修）已可用。四项腾讯云配置齐全时，抠图走数据万象 GoodsMatting。

当前待补充：

- 图片上传在 Mock 模式使用本地 Data URL；真实对象存储需提供 `POST /api/uploads`。
- 智能选区、图片工作站、自由画布的图片和视频生成，以及转比例里需要补边的智能扩展，仍没有真实 Provider。`POST /api/tasks` 在真实模式只接受邮件助手。智能裁剪的主体检测仍是固定模拟框。转比例再到加水印还没有流水线入口。
